...
modres <- read.csv(file="case_study/results/predictions_ranked.csv", sep=';')
modres<-modres[, -1]
# Convert ENSG ids
...
genecards <- read.csv(file="case_study/results/interpretation/genecards/GeneCards_gene_names.csv")
...
save(genecards_g2ensg, file="case_study/results/interpretation/genecards/genecards_g2ensg.RData")
...
save(genecards_modres,file="case_study/results/interpretation/genecards_modres.RData")
...
protorray_genes<-read.csv(file="case_study/results/interpretation/protoarray_alz/genes_protoarray_alz.csv")
...
save(protorray_g2ensg, file="case_study/results/interpretation/protoarray_alz/protorray_g2ensg.RData")
# Overlap with the genes of interest
protoarray_modres <- merge(modres, protorray_g2ensg, by.x = "ensg", by.y = "ensg", all.y = F)
save(protoarray_modres,file="case_study/results/interpretation/protoarray_modres.RData")


############ 3. Genes from GWAX  
# Genes extracted from the article Liu et al. (published)
liu<-read.csv(file="case_study/results/interpretation/liu/liu_alzgenes_sup_table4.csv")
liu_g2ensg <- gconvert(liu$gene_name)
liu_g2ensg <-liu_g2ensg[, c(4,5)]
colnames(liu_g2ensg) <-c("ensg", "gene_name")
save(liu_g2ensg, file="case_study/results/interpretation/liu/liu_g2ensg.RData")
# Overlap with the genes of interest
liu_modres <- merge(modres, liu_g2ensg, by.x = "ensg", by.y = "ensg", all.y = F)
save(liu_modres,file="case_study/results/interpretation/liu_modres.RData")


############ 4. Genes from GWAS 
# Genes extracted from the article Jansen et al.  (bioRxiv, not peer-reviewed yet)
jansen <- read.csv(file="case_study/results/interpretation/jansen/jansen_alzgenes_sup_table5.csv", sep=";")
...
save(jansen_g2ensg, file="case_study/results/interpretation/jansen/jansen_g2ensg.RData")
# Overlap with the genes of interest
jansen_modres <- merge(modres,jansen_g2ensg, by.x = "ensg", by.y = "ensg", all=F)
save(jansen_modres,file="case_study/results/interpretation/jansen_modres.RData")

############ 5. Genes from GWAS 
# Genes extracted from the article Kunkle et al. (bioRxiv, not peer-reviewed yet)
kunkle <- read.csv(file="case_study/results/interpretation/kunkle/kunkle_alzgenes.csv")
...
save(kunkle_g2ensg, file="case_study/results/interpretation/kunkle/kunkle_g2ensg.RData")
# Overlap with the genes of interest
kunkle_modres <- merge(modres,kunkle_g2ensg, by.x = "ensg", by.y = "ensg", all=F)
save(kunkle_modres,file="case_study/results/interpretation/kunkle_modres.RData")


############ 5. Genes from GWAS 
# Genes extracted from the article Marioni et al.(published)
marioni <- read.csv(file="case_study/results/interpretation/marioni/marioni_alzgenes.csv")
...
save(marioni_g2ensg, file="case_study/results/interpretation/marioni/marioni_g2ensg.RData")
# Overlap with the genes of interest
marioni_modres <- merge(modres,marioni_g2ensg, by.x = "ensg", by.y = "ensg", all=F)
save(marioni_modres,file="case_study/results/interpretation/marioni_modres.RData")


############ 6. Genes from GWAS, TWAS, splicing analysis
# Genes extracted from the article Raj et al. (published)
raj <- read.csv(file="case_study/results/interpretation/raj/raj_alzgenes_table10.csv")
...
save(raj_g2ensg, file="case_study/results/interpretation/raj/raj_g2ensg.RData")
# Overlap with the genes of interest
raj_modres <- merge(modres,raj_g2ensg, by.x = "ensg", by.y = "ensg", all=F)
save(raj_modres,file="case_study/results/interpretation/raj_modres.RData")
